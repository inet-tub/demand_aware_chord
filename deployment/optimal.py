...
opt_mod.setParam("IterationLimit",10000)
...
def gurobi_optimal(G,D):
    ...
    #opt_mod.addConstrs((dist[i,j] <= dist[i,u] + dist[u,j] for i in G.nodes for j in G.nodes for u in G.nodes),name="c-dist2")
    ...
    opt_mod.setObjective(sum([x[i,j,:,:].sum()*D[i][j] for (i,j) in nodesPairListNoDuplication]), GRB.MINIMIZE)
    opt_mod.update()
    opt_mod.optimize()
